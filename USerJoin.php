...
<head>
    ...
    <link href="USerJoin.css" rel="stylesheet">
  ...
  <body>
    ...
    <div class="padd-box">
        ...
        <section class="section clear-mrg">
            <h2 class="title-lg text-upper">已報名活動</h2>
			<?php
				$sql = "SELECT * FROM applicationform,event where applicationform.eventID=event.eventID && applicationform.cusID=$cusID order by applicationform.add_time desc" ;
				$data = mysql_query($sql) or die(mysql_error());
            ...
            <!---------------------------內文開始(小心其他框架)----------------->
                <div class="ref-box brd-btm hreview">
                    <div class="ref-avatar">
                        <img alt="" src="upload/<?=$row['eventPic']?>" class="avatar avatar-54 photo" height="54" width="54">
                    </div>
					
                    <div class="ref-info">
                        <div class="ref-author">
                            <strong><?=$row['eventName']?></strong></p>
                            <span>報名日期:&nbsp <?=date('Y-m-d',strtotime($row['add_time']))?></span></p>
                            <span>活動日期:&nbsp <?=$row['startDate']?></span></p>
                            <span>活動地點:&nbsp <?=$row['address']?></span>
                        </div>
                        <a href="Event.php?f=<?=$row['eventID']?>" class="myButton" target=blank>查看活動頁面</a>
                    </div>
                </div><!-- .ref-box -->
